...
class DataFeeder(threading.Thread):
  ...
  def __init__(self, coordinator, metadata_filename, hparams):
    ...
    for item in self.metadata_filename:
      ...
      for item in metadata_tmp:
        metadata_tmp2.append([os.path.join(dirname, item[0]),
                              os.path.join(dirname, item[1]),
                              item[2],
                              item[3],])
                              #item[4]])
    ...
    self.inputs, self.input_lengths, self.mel_targets, self.linear_targets, self.wav = queue.dequeue()
    self.inputs.set_shape(self._placeholders[0].shape)
    self.input_lengths.set_shape(self._placeholders[1].shape)
    self.mel_targets.set_shape(self._placeholders[2].shape)
    self.linear_targets.set_shape(self._placeholders[3].shape)
    self.wav.set_shape(self._placeholders[4].shape)
  ...
  def _get_next_example(self):
    ...
    return (input_data, mel_target, linear_target, len(input_data), wav)
...
def _prepare_batch(batch, outputs_per_step):
  ...
  wav = _prepare_inputs([x[4] for x in batch])
  mel_targets = _prepare_targets([x[1] for x in batch], outputs_per_step)
  linear_targets = _prepare_targets([x[2] for x in batch], outputs_per_step)
  return (inputs, input_lengths, mel_targets, linear_targets, wav)
