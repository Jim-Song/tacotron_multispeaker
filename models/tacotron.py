...
from text.symbols import symbols, symbols2
...
class Tacotron():
  ...
  def initialize(self, inputs, input_lengths, mel_targets=None, linear_targets=None, embedding_channels=256):
    ...
    with tf.variable_scope('inference') as scope:
      ...
      # Embeddings
      if not hp.chinese_symbol:
        embedding_num = len(symbols)
      else:
        embedding_num = len(symbols2)
      print('embedding_num')
      print(embedding_num)
      try:
        embedding_channels = self._hparams.embedding_channels
      except:
        pass
      embedding_table = tf.get_variable(
        'embedding', [embedding_num, embedding_channels], dtype=tf.float32,
